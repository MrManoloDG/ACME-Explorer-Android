...
public class SignupActivity extends AppCompatActivity {
    ...
    private AutoCompleteTextView login_pass_et_confirmation;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String emailParams = getIntent().getStringExtra(EMAIL_PARAM);

        login_email_et = findViewById(R.id.login_email_et);
        login_pass_et = findViewById(R.id.login_password_et);
        login_pass_et_confirmation = findViewById(R.id.login_password_et_confirmation);

        login_email = findViewById(R.id.login_email);
        login_pass = findViewById(R.id.login_password);
        login_pass_confirmation = findViewById(R.id.login_password_confirmation);

        login_email_et.setText(emailParams, false);

        findViewById(R.id.login_signup_button).setOnClickListener(l -> {
            if (login_email_et.getText().length() == 0) {
            ...
            } else if (login_pass_et_confirmation.getText().length() == 0){
                login_pass_confirmation.setErrorEnabled(true);
                login_pass_confirmation.setError(getString(R.string.signup_error_pass));
            } else if (!login_pass_et_confirmation.getText().toString().equals(login_pass_et.getText().toString())) {
                login_pass.setErrorEnabled(true);
                login_pass.setError(getString(R.string.signup_error_pass_not_equal));
            } else {
                FirebaseAuth.getInstance().createUserWithEmailAndPassword(login_email_et.getText().toString(), login_pass_et.getText().toString()).addOnCompleteListener(task -> {
                    if (task.isSuccessful()){
                        ...
                        Toast.makeText(this, "An error was appear during register", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
